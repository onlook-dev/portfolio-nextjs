    ...
    <nav className="px-8 max-w-3xl mx-auto mt-32">
      ...
      >
        ...
        <div className="space-y-3.5">
          ...
          {/* NAV LINKS */}
          <div className="flex gap-5 text-primary-gray text-nav-primary-list justify-center items-center flex-col">
            {navLinks.map((link) => (
              <p
                className={`${
                  pathname === link.route
                ...
                key={link.id}
              >
                <Link href={link.route} className="relative text-[#e8e8e8]">
                  {link.route === pathname && (
                    <motion.span
                      layoutId="underline"
                      className={clsx(
