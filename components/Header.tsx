      ...
      >
        ...
        <div className="space-y-3.5">
          ...
          <h1 className="text-primary-white text-nav-primary-heading">
            <span className={`${poppins} text-8xl font-bold`}>Joshua</span>
          </h1>

          {/* NAV LINKS */}
          <div className="flex flex-col items-center gap-5 text-primary-gray text-nav-primary-list">
            {navLinks.map((link) => (
              <p
                className={`${
                  pathname === link.route
                ...
                key={link.id}
              >
                <Link href={link.route} className="relative text-white text-[#ffffff]">
                  {link.route === pathname && (
                    <motion.span
                      layoutId="underline"
                      className={clsx(
