      ...
      >
        ...
        <div className="space-y-3.5">
          ...
          <h1 className="text-primary-white text-nav-primary-heading">
            <span className={`${poppins} text-8xl font-bold`}>Matt's</span>
          </h1>

          {/* NAV LINKS */}
          <div className="flex flex-row m-6 p-1 gap-6 text-primary-gray text-nav-primary-list">
            {navLinks.map((link) => (
              <p
                className={`${
                  pathname === link.route
                ...
                key={link.id}
              >
                <Link href={link.route} className="relative text-[#d9d9d9] text-[#d9d9d9] text-[#d9d9d9]">
                  {link.route === pathname && (
                    <motion.span
                      layoutId="underline"
                      className={clsx(
