      ...
      >
        ...
        <div className="space-y-3.5">
          ...
          <h1 className="text-primary-white text-nav-primary-heading">
            <span className={`${poppins} text-8xl font-bold`}>Charlie</span>
          </h1>

          {/* NAV LINKS */}
          <div className="flex gap-[10px] text-primary-gray text-nav-primary-list p-[30px] flex-col items-start">
            {navLinks.map((link) => (
              <p
                className={`${
                  pathname === link.route
                ...
                key={link.id}
              >
                <Link href={link.route} className="relative text-[#ffffff]">
                  {link.route === pathname && (
                    <motion.span
                      layoutId="underline"
                      className={clsx(
