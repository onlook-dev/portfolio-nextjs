      ...
      >
        ...
        <div className="space-y-3.5">
          ...
          <h1 className="text-primary-white text-nav-primary-heading">
            <span className={`${poppins} text-8xl font-bold`}>Nathan</span>
          </h1>

          {/* NAV LINKS */}
          <div className="flex gap-5 text-primary-gray text-nav-primary-list items-center flex-col justify-start">
            {navLinks.map((link) => (
              <p
                className={`${
                  pathname === link.route
                ...
                key={link.id}
              >
                <Link href={link.route} className="relative text-[#ff0f0f] text-[#ff0f0f] text-[#ff0f0f]">
                  {link.route === pathname && (
                    <motion.span
                      layoutId="underline"
                      className={clsx(
